...
class AUDIOLIB:
    ...
    def _recognize_words(self, file_path):
        """Recognizes words from audio file using Wav2Vec2 model."""
        try:
            audio_input, _ = sf.read(file_path)
            input_values = self.processor(audio_input, return_tensors="pt", sampling_rate=self.samplerate).input_values
